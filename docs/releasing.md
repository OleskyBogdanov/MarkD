...
Неподписанная NSIS-установка и запуск Windows x64, а также ad-hoc пакеты macOS arm64/Intel проверены в [GitHub Actions](https://github.com/OleskyBogdanov/MarkD/actions/runs/35523121204): по 45 unit и 43 Electron E2E на каждом runner. Windows runner — Server 2022; приёмка на пользовательских Windows 10/11 остаётся частью выпуска. Скриншоты и PDF обеих ОС просмотрены, но автоматические визуальные baseline пока не утверждены.

Developer ID/notarization, Windows signing, HTTPS feed и обновление N → N+1 двух подписанных установок ещё не проверены. Прохождение verify workflow подтверждает обычные пакеты, но не заменяет эти ворота production-выпуска.
